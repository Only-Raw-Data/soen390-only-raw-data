...
import { Building, SGW_BUILDINGS, LOYOLA_BUILDINGS } from '@/constants/buildings';
...
function getCacheFile(campus: 'SGW' | 'Loyola'): File {
    return new File(getCacheDir(), `${campus.toLowerCase()}_polygons.json`);
}

async function getCachedPolygons(campus: 'SGW' | 'Loyola'): Promise<BuildingPolygon[] | null> {
    try {
...
async function cachePolygons(campus: 'SGW' | 'Loyola', polygons: BuildingPolygon[]): Promise<void> {
    try {
...
export function useBuildingPolygons(campus: 'SGW' | 'Loyola') {
